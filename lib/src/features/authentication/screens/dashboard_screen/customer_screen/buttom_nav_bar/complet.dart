...
class _MyWidgetState extends State<complete_order> {
  List<dynamic> orders = [];
  int totalOrders = 0;
  ...
  void _searchData() {
    if (selectedOption == 'Today') {
      ...
      String endDate =
          '${now.year}-${_formatNumber(now.month)}-${_formatNumber(now.day)}';
      searchOrders(startDate, endDate, now);
    }
  }

  void searchOrders(String startDate, String endDate, DateTime currentDate) {
    final DateFormat dateFormat = DateFormat('yyyy-MM-dd');

    _databaseRef
        .orderByChild('customerDelivery')
        .endAt(startDate)
        .onValue
        .listen((event) {
        ...
        setState(() {
          orders = data.values
            ...
            return deliveryDate.isBefore(currentDate);
              ...
              .toList()
            ..sort((a, b) {
              final DateTime deliveryDateA =
              _parseDate(a['customerDelivery'], dateFormat);
              final DateTime deliveryDateB =
              _parseDate(b['customerDelivery'], dateFormat);
              return deliveryDateA.compareTo(deliveryDateB);
            });
          calculateTotals();
        });
      } else {
